	...
	public void process() {
		...
		while (bool) {
			...
			case 1:
				...
				member.login();
				if ((MemberUtil.getLoginedId().equals("admin"))) {
					System.out.println("[ " + MemberUtil.getLoginedId() + " ] 관리자  계정으로  로그인 되었습니다.");
					admin();
				} else {
			...
			case 2:
				...
				member.login();
				if (MemberUtil.getLoginedId() == "") {
					break;
				} else {
					System.out.println("[ " + MemberUtil.getLoginedId() + " ] 님 로그인 되었습니다.");
					MemberMenu();
				}
				break;
			case 3:
